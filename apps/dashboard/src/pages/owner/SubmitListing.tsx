...
function PhotoSection({ tone, uploadedUrls, thumbnail, uploading, fileInputRef, onFileChange, onRemove, onSetThumbnail, label = 'Click to upload photos' }: {
  tone: string; uploadedUrls: string[]; thumbnail: string | null; uploading: boolean
  fileInputRef: React.RefObject<HTMLInputElement | null>
  onFileChange: (e: React.ChangeEvent<HTMLInputElement>) => void
  onRemove: (url: string) => void; onSetThumbnail: (url: string) => void; label?: string
}) {
  return (
    <div>
      ...
      <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}
        ...
        <span className="text-[13px] font-semibold" style={{ color: tone }}>{uploading ? 'Uploading…' : label}</span>
...
function FormSections({
  ...
  form: Record<string, unknown>; set: (f: string, v: unknown) => void; toggleFeature: (f: string) => void
  tone: string; uploadedUrls: string[]; thumbnail: string | null; uploading: boolean
  fileInputRef: React.RefObject<HTMLInputElement | null>
  handleFileChange: (e: React.ChangeEvent<HTMLInputElement>) => void
  removeImage: (url: string) => void; setThumbnail: (url: string) => void; photoLabel?: string
  priceCurrency: 'USD' | 'DOP'; setPriceCurrency: (c: 'USD' | 'DOP') => void; dopRate: number
}) {
  ...
  return (
    <>
      ...
      {/* 4 — Financials (dynamic) */}
      <Sec n={n()} title="Financials" tone={tone}>
        {isRent ? (
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Toggle value={association} onChange={v => { set('association', v); if (!v) set('association_fee', '') }} label="Association Fee" tone={tone} />
              {association && (
                <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              {hoa && (
                <div>
                  ...
                  <input className={inp} type="number" value={form.hoa_fee as string} onChange={e => set('hoa_fee', e.target.value)} placeholder="e.g. 350" min="0" />
                </div>
              )}
            </div>
          </div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {customFeatures.map(f => (
                  <span key={f} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-semibold border"
                    ...
                    {f}
                    <button type="button" onClick={() => toggleFeature(f)} className="ml-0.5 rounded-full w-3.5 h-3.5 grid place-items-center text-[10px] cursor-pointer border-0" style={{ background: tone, color: 'white' }}>×</button>
                  </span>
                ))}
              </div>
            )}
            <div className="flex gap-2 mt-2">
              <input className={inp + ' flex-1'} type="text" value={customInput} onChange={e => setCustomInput(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addCustomFeature() } }} placeholder="Add custom feature…" maxLength={50} />
              <button type="button" onClick={addCustomFeature} disabled={!customInput.trim()}
                className="px-4 py-2.5 rounded-lg text-[13px] font-semibold border-0 cursor-pointer disabled:opacity-40 transition-opacity" style={{ background: tone, color: 'white' }}>Add</button>
            </div>
          </div>
        </div>
      </Sec>
      ...
      <Sec n={n()} title="Media & Location" tone={tone}>
        <div className="space-y-4">
          <PhotoSection tone={tone} uploadedUrls={uploadedUrls} thumbnail={thumbnail} uploading={uploading}
            fileInputRef={fileInputRef} onFileChange={handleFileChange} onRemove={removeImage} onSetThumbnail={setThumbnail} label={photoLabel} />
            ...
            <div className="space-y-2">
              {videoLinks.map((url, i) => (
                <div key={i} className="flex gap-2">
                  <input className={inp + ' flex-1'} type="url" value={url} onChange={e => updateVideoLink(i, e.target.value)} placeholder="https://youtube.com/watch?v=…" />
                  <button type="button" onClick={() => removeVideoLink(i)} className="px-3 py-2.5 rounded-lg border border-line bg-white text-dim cursor-pointer hover:text-red-500 hover:border-red-300 transition-colors"><X size={14} /></button>
                </div>
              ))}
            </div>
            <button type="button" onClick={addVideoLink} className="mt-2 flex items-center gap-1.5 text-[13px] font-semibold cursor-pointer border-0 bg-transparent transition-opacity hover:opacity-70" style={{ color: tone }}>
...
export function OwnerSubmitListing({ go, tone }: { go: (v: string) => void; tone: string }) {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <button type="button" onClick={() => go('listings')} className="flex items-center gap-1.5 text-dim text-[13px] mb-6 bg-transparent border-0 cursor-pointer hover:text-ink transition-colors">
        <ArrowLeft size={14} /> Back to Listings
      </button>
      <form onSubmit={handleSubmit} className="space-y-4">
        <FormSections form={form} set={set} toggleFeature={toggleFeature} tone={tone}
...
export function OwnerEditListing({ listing, tone, onBack, onSaved }: {
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const msg = listing.status === 'rejected' ? 'Listing resubmitted for review!'
        : listing.status === 'active' ? 'Changes submitted for review. Your listing stays live until approved.' : 'Changes saved!'
      toast.success(msg)
      onSaved(updated)
    } catch { toast.error('Something went wrong. Please try again.') }
    finally { setSubmitting(false) }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <button type="button" onClick={onBack} className="flex items-center gap-1.5 text-dim text-[13px] mb-4 bg-transparent border-0 cursor-pointer hover:text-ink transition-colors">
      ...
      {isActive && (
        <div className="mb-5 flex items-start gap-3 px-4 py-3.5 rounded-xl border text-[13px]"
          ...
          <span>This listing is <strong>live</strong>. Your changes will be reviewed by an admin before going live — the current version stays visible on search until approved.</span>
        </div>
      )}
      <form onSubmit={handleSubmit} className="space-y-4">
        <FormSections form={form} set={set} toggleFeature={toggleFeature} tone={tone}
        ...
        <div className="flex gap-3 pt-2 pb-6">
          ...
          <button type="submit" disabled={submitting} className="px-8 py-3 rounded-full text-white text-[13.5px] font-bold border-0 cursor-pointer disabled:opacity-50 transition-opacity" style={{ background: tone }}>
            {submitting ? 'Submitting…'
              : isActive ? 'Submit Changes for Review'
              : listing.status === 'rejected' ? 'Save & Resubmit for Review'
              : 'Save Changes'}
          </button>
        </div>
      </form>
