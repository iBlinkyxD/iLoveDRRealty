...
import OwnerDash    from './views/OwnerDash'
import RealtorDash  from './views/RealtorDash'
import AdminDash    from './views/AdminDash'

export type Role = 'Buyer' | 'Investor' | 'Owner' | 'Realtor' | 'Admin'

const ROLES: { key: Role; icon: string; desc: string; tone: string; email: string }[] = [
...
const VIEW_MAP: Record<Role, React.ComponentType<{ go: (v: string) => void }>> = {
  Buyer:    BuyerDash,
...
export default function App() {
  ...
  const initialRole = roleParam && ROLES.some(r => r.key === roleParam) ? roleParam : null
  ...
  if (!role) {
    return <RoleSelect onSelect={r => { setRole(r); setView('home') }} />
  }

  const View = VIEW_MAP[role]
  return (
    <DashLayout role={role} view={view} go={setView} onLogout={handleLogout}>
      <View go={setView} />
    </DashLayout>
  )
}
