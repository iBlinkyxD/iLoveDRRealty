...
const DIFF_LABELS: Record<string, string> = {
  ...
  co_listing_agent_name: 'External Agent Name', co_listing_brokerage_email: 'Brokerage Email', co_listing_brokerage_phone: 'Brokerage Phone',
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-150 max-w-full bg-paper z-50 flex flex-col shadow-2xl">
        ...
        <div className="px-5 py-3 bg-line/20 border-b border-line shrink-0 flex items-center justify-between">
          <div>
            <div className="text-[10.5px] font-semibold text-dim uppercase tracking-wide">{t('edit_diff.submitted_by')}</div>
          ...
          <div className="text-right">
            <div className="text-[10.5px] font-semibold text-dim uppercase tracking-wide">{t('edit_diff.submitted')}</div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {changed.length === 0 ? (
            <div className="text-center py-10 text-sm text-dim">{t('edit_diff.no_changes')}</div>
          ) : (
            <div className="space-y-3">
              <div className="grid grid-cols-[1fr_1fr_1fr] gap-2 text-[10px] font-semibold text-dim uppercase tracking-wide pb-1 border-b border-line">
                <div>{t('edit_diff.col_field')}</div>
                <div>{t('edit_diff.col_current')}</div>
                <div style={{ color: TONE }}>{t('edit_diff.col_proposed')}</div>
              </div>
              {changed.map(key => {
          ...
          {(() => {
            ...
            return (
              <div className="mt-4">
                <div className="text-[10.5px] font-semibold text-dim uppercase tracking-wide mb-2">{t('edit_diff.photos_compare')}</div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <div className="text-[11px] text-dim mb-1.5">{t('edit_diff.current_count', { count: curImgs.length })}</div>
                    <div className="grid grid-cols-3 gap-1">
                      {curImgs.slice(0, 6).map((u, i) => <img key={i} src={u} alt="" className="w-full h-14 object-cover rounded-lg" />)}
                    </div>
                  </div>
                  <div>
                    <div className="text-[11px] mb-1.5 font-medium" style={{ color: TONE }}>{t('edit_diff.proposed_count', { count: propImgs.length })}</div>
                    <div className="grid grid-cols-3 gap-1">
          ...
          {rejectOpen && (
            <div className="mt-4 flex gap-2">
              <input
                ...
                placeholder={t('edit_diff.reason_ph')}
        ...
        <div className="border-t border-line px-5 py-4 flex gap-3 shrink-0">
          ...
          >
            <Check size={14} strokeWidth={2.5} /> {t('edit_diff.approve_btn')}
          </button>
          <button
          ...
          >
            <X size={14} strokeWidth={2.5} /> {t('edit_diff.reject_btn')}
          </button>
