...
export function AddUserPanel({ open, onClose, onCreated }: Props) {
  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    try {
      ...
      toast.success(t('add_user_panel.toast_created'))
    } catch (err: any) {
      if (err?.response?.status === 409) toast.error(t('add_user_panel.toast_duplicate'))
      else toast.error(t('add_user_panel.toast_error'))
    } finally {
      setWorking(false)
    }
  }
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-full sm:w-96 bg-white shadow-2xl z-50 flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-line">
          <div>
            <div className="font-bold text-[16px] text-ink">{t('add_user_panel.title')}</div>
            <div className="text-[12px] text-dim mt-0.5">{t('add_user_panel.subtitle')}</div>
          </div>
        ...
        <form id="add-user-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
          <div>
            <label className={LABEL_CLS}>{t('add_user_panel.label_name')}</label>
            <input
              required
              className={INPUT_CLS}
          ...
          <div>
            <label className={LABEL_CLS}>{t('add_user_panel.label_email')}</label>
            <input
              type="email"
              required
              className={INPUT_CLS}
          ...
          <div>
            <label className={LABEL_CLS}>{t('add_user_panel.label_password')}</label>
            <input
              type="password"
              required
              minLength={8}
              ...
              placeholder={t('add_user_panel.ph_password')}
            />
          </div>
          <div>
            <label className={LABEL_CLS}>{t('add_user_panel.label_role')}</label>
            <select
              className={INPUT_CLS}
              value={form.role}
        ...
        <div className="px-5 py-4 border-t border-line flex gap-2">
          ...
          >
            {working ? t('add_user_panel.creating') : t('add_user_panel.create')}
          </button>
        </div>
      </div>
    </>
