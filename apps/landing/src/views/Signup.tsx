...
export default function Signup() {
  ...
  const needsVerify = role === 'Owner' || role === 'Realtor'
  ...
  return (
    <div className="min-h-[calc(100vh-74px)] grid grid-cols-2 font-sans">
      ...
      <div className="flex flex-col py-15 px-14 relative overflow-hidden"
        style={{ background: 'linear-gradient(155deg, #00102e 0%, #0a1f50 55%, #003a73 100%)' }}>
        <div className="absolute -top-20 -right-20 w-100 h-100 rounded-full"
          style={{ background: 'radial-gradient(circle, rgba(240,168,0,.14) 0%, transparent 65%)' }} />
        <div className="absolute -bottom-15 -left-10 w-75 h-75 rounded-full"
          style={{ background: 'radial-gradient(circle, rgba(225,15,31,.12) 0%, transparent 65%)' }} />
        ...
        <div className="relative bg-white/6 border border-white/10 rounded-4 pt-5.5 px-5.5 pb-4.5">
          <div className="flex items-center gap-2.5 mb-4">
            <div className="w-9 h-9 rounded-2.25 grid place-items-center text-4.5"
          ...
          {needsVerify && (
            <div className="mt-4 py-2.5 px-3 bg-gold/12 border border-gold/25 rounded-2.5 text-xs text-white/70 leading-normal">
      ...
      <div className="bg-paper2 flex items-center justify-center py-12 px-7 overflow-y-auto">
        <div className="w-full max-w-110">
          ...
          <div className="bg-paper border border-line rounded-4 p-7">
                ...
                {ROLES.map(r => {
                  ...
                  return (
                    <button key={r.key} onClick={() => setRole(r.key)}
                      className="py-2.5 px-3 rounded-2.5 cursor-pointer text-left font-sans transition-all duration-150"
                      style={{
            ...
            <div className="grid grid-cols-2 gap-3 mb-3.5">
              ...
              <div>
                ...
                <input type="tel" placeholder="+1 (809) …" className={inputCls} />
              </div>
            </div>
                ...
                {(['Resident', 'Non-resident'] as const).map(r => {
                  ...
                  return (
                    <button key={r} onClick={() => setResidency(r)}
                      className={`flex-1 py-2.25 rounded-2.5 cursor-pointer font-sans text-3.25 transition-all duration-150 ${on ? 'border border-sea bg-sea text-white font-bold' : 'border border-line bg-paper text-ink2 font-medium'}`}>
                      {r}
                    </button>
                  )
                })}
            ...
            <button
              onClick={() => agreed && setDone(true)}
              className={`w-full py-3.25 rounded-full border-none font-sans text-3.75 font-bold transition-colors duration-150 ${agreed ? 'bg-coral text-white cursor-pointer' : 'bg-line-soft text-dim cursor-not-allowed'}`}
            >
              Create account
            </button>
            ...
            <div className="flex gap-2.5">
              {[['🇬 Google', '#fff', '#00102e', '#e4ddcf'], ['📘 Facebook', '#1877F2', '#fff', '#1877F2']].map(([l, bg, fg, bd], i) => (
                <button key={i} className="flex-1 py-2.75 rounded-full font-sans text-[13.5px] font-semibold cursor-pointer"
                  style={{ background: bg as string, color: fg as string, border: `1px solid ${bd}` }}>{l}</button>
              ))}
            </div>
          </div>
